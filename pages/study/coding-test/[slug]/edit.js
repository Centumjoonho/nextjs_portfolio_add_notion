...
export default function EditPost() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          problemNo: post.problemNo || '',
          difficulty: post.difficulty || '',
          tags: post.tags.join(', '),
          summary: post.summary || '',
          contentMarkdown: post.contentMarkdown,
          codeBlock: post.codeBlock || '',
          retrospective: post.retrospective || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push(`/study/coding-test/${slug}`);
      } else {
  ...
  return (
    <Layout>
      <Head>
        <title>글 수정 - 코딩 개발 공부 - 센텀준호</title>
      </Head>
      <div className="container mx-auto px-4 py-8">
        <h1 className="mb-6 text-3xl font-bold text-slate-900 dark:text-white">글 수정</h1>
